...
window.ig = {
	...
	_boot: function() {
		...
		//PR 06/07/14 Added ua variable to help check for other device types
		var ua = navigator.userAgent; // for ref as impact already does this

    if (ig.ua.android) {
        var parts = ua.slice(ua.indexOf("Android") + 8).split(/\s*[;)(]\s*/);
        ig.ua.androidversion = parts[0];
    }

    ig.ua.blackberry = (/BlackBerry/i.test(ua) && /WebKit/i.test(ua)) || /BB/i.test(ua) || /RIM/i.test(ua);

    ig.ua.winPhone = /Windows Phone/i.test(navigator.userAgent);
    ig.ua.iOS = ig.ua.iPhone || ig.ua.iPad;
    ig.ua.mobile = ig.ua.iOS || ig.ua.android || ig.ua.winPhone || ig.ua.blackberry || /mobile/i.test(navigator.userAgent);
